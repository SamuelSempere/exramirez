...
function formatDate(date) {
 ...
 // Usa tu base64 completo
    ...
    const addText = (text, y, size = fontSize, color = rgb(0, 0, 0)) => {
        page.drawText(text, { x: 50, y, size, color });
    };

    const addHeader = () => {
        addText('Solicitud de Material', yPosition, titleFontSize, rgb(3 / 255, 61 / 255, 131 / 255));
        yPosition -= (lineHeight * 2);
        addText(`Solicitado por: ${username}`, yPosition, titleFontSize, rgb(3 / 255, 61 / 255, 131 / 255));
        yPosition -= (lineHeight * 2);
    };
    ...
    if (formData.materiales && formData.materiales.length > 0) {
      yPosition -= lineHeight * 2; // Añadir espacio antes del título
      addText("Detalle de Materiales:", yPosition, titleFontSize, rgb(3 / 255, 61 / 255, 131 / 255));
      yPosition -= lineHeight;
...
console.log(userEmail)
        ...
        console.error('Error en la generación del PDF o envío:', error);
